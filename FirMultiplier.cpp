...
xip_fir_v7_2_rld_packet fir_real_multiplier_rld;		// reload-пакет для установки коэффициента a0
xip_fir_v7_2_cnfg_packet fir_real_multiplier_cnfg_rld;	// config-пакет для установки коэффициента a0
xip_array_uint* fir_real_multiplier_cnfg_fsel;
...
int init_fir_real_multiplier()
{
	...
	fir_real_multiplier_cnfg.reloadable = 1;
	fir_real_multiplier_cnfg.num_coeffs = 1;
	fir_real_multiplier_cnfg.coeff_width = 26;
	fir_real_multiplier_cnfg.coeff_fract_width = 10;
	fir_real_multiplier_cnfg.quantization = XIP_FIR_QUANTIZED_ONLY;
	fir_real_multiplier_cnfg.output_rounding_mode = XIP_FIR_FULL_PRECISION;
	fir_real_multiplier_cnfg.data_width = 26;
	fir_real_multiplier_cnfg.data_fract_width = 10;
	...
	// Конфигурационный пакет. Необходим для применения reload-пакета
	fir_real_multiplier_cnfg_fsel = xip_array_uint_create();
	xip_array_uint_reserve_dim(fir_real_multiplier_cnfg_fsel, 1);
	fir_real_multiplier_cnfg_fsel->dim_size = 1;
	fir_real_multiplier_cnfg_fsel->dim[0] = fir_real_multiplier_cnfg.num_channels;
	fir_real_multiplier_cnfg_fsel->data_size = fir_real_multiplier_cnfg_fsel->dim[0];
	if (xip_array_uint_reserve_data(fir_real_multiplier_cnfg_fsel, fir_real_multiplier_cnfg_fsel->data_size) != XIP_STATUS_OK) {
		printf("Unable to reserve data!\n");
		return -1;
	}

	fir_real_multiplier_cnfg_rld.fsel = fir_real_multiplier_cnfg_fsel;
	fir_real_multiplier_cnfg_rld.fsel->data[0] = 0;

	// Резервируем память в reload-пакете для коэффициента a0
...
{
	if (xip_array_real_destroy(fir_real_multiplier_rld.coeff) != XIP_STATUS_OK) {
		return -1;
	}
	if (xip_array_uint_destroy(fir_real_multiplier_cnfg_fsel) != XIP_STATUS_OK) {
		return -1;
	}
